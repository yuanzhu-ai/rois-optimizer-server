...
def test_rule_manday_byCrew():
    """测试Rule优化器 - manday_byCrew类型"""
    print("\n" + "="*80)
    print("测试 Rule 优化器 - manday_byCrew类型")
    print("="*80 + "\n")
...
if __name__ == "__main__":
    ...
    try:
        # 测试Rule manday_byCrew
        task_id = test_rule_manday_byCrew()
        if task_id:
            print(f"\n✓ Rule任务已启动，任务ID: {task_id}")
            time.sleep(3)
        ...
        print("\n" + "="*80)
        print("检查工作目录命名:")
        print("Rule目录格式: {category}_{timestamp}_{taskId}")
        print("="*80)
